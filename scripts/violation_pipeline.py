...
class ViolationPipeline:
    ...
    def process_frame(self, frame, frame_idx=None, timestamp=None):
        ...
        # 2. gate: only run object detection + violation logic when barrier is closing/closed
        if barrier_state not in ("closing", "closed"):
            return result

        # 3. object detection
        pred = self.object_model.predict(frame, conf=self.object_conf, iou=self.object_iou, verbose=False)[0]
        detections = [
            {'class': self.object_model.names[int(b.cls[0])], 'box': b.xyxy[0].tolist(), 'conf': float(b.conf[0])}
            for b in pred.boxes
        ]
        ...
        # 5. ROI filtering
        detections = filter_detections_by_roi(detections, self.roi_polygon, frame.shape, self.roi_overlap_threshold)
